      ...
      {list.map((p) => (
        ...
        >
          ...
          <span className="font-medium text-orange-400">
            {toWon(p.amount)} P ({p.percent.toFixed(1)}%)
          </span>
        </li>
      ))}
    </ul>
...
function WeeklyButton() {
  ...
  return (
    ...
    >
      {isWeeklyLoading ? "주간 실행 중..." : "주간 실행 (세금 3% + 채굴)"}
    </button>
  );
}
    ...
    <section className="mb-8 space-y-6">
      ...
      {activeGoals.length === 0 ? (
        <p className="rounded-xl border border-white/10 bg-slate-900/50 px-4 py-3 text-sm text-gray-400">
          활성 펀딩 목표가 없습니다. 아래에서 새 목표를 만들면 세금과 채굴 나머지가 적립됩니다.
        </p>
      ) : (
  ...
  return (
    <article className="rounded-xl border border-orange-400/30 bg-slate-900/70 p-5 shadow-lg">
      ...
      <p className="mt-2 text-2xl font-extrabold text-orange-400">
        ...
        <span className="text-lg font-normal text-gray-400">/ {toWon(goal.target_amount)} P</span>
      </p>
  ...
  return (
    <article className="rounded-xl border border-slate-600/50 bg-slate-900/50 p-5 shadow-lg">
      ...
      <p className="mt-2 text-lg font-bold text-white">
        목표 {toWon(goal.target_amount)} P 달성
        {totalContributed > 0 && (
          <span className="ml-2 text-sm font-normal text-gray-400">
            (학생 기부 {toWon(totalContributed)} P)
          </span>
        )}
      </p>
      {(burnedAmount ?? 0) > 0 && (
